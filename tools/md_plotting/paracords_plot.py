...
def main(infile, col_dimensions, categorical, col_color):
    """
    ...
    for col in col_dimensions:
        ...
        if categorical == 'boolfalse' and all(type(e) is int for e in values ):
            dimensions.append(
                dict(   values = values,
                        tickformat = ",.2r",
                        label = df.columns[col])
            )
        elif categorical == 'boolfalse' and all(type(e) is float for e in values ):
            dimensions.append(
                dict(   values = values,
                        tickformat = "g",
                        label = df.columns[col])
...
if __name__ == "__main__":
    ...
    aparser.add_argument( "-t", "--categorical_datatype", dest="categorical")
    aparser.add_argument( "-c", "--col_color", dest="col_color")
    args = aparser.parse_args()

    main(args.infile, args.col_dimensions, args.categorical, args.col_color)